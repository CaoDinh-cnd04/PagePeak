...
type Props = {
  ...
  /** Có quyền dùng AI - nếu false thì nút AI bị khóa, click sẽ gọi onUpgradeClick */
  hasAi?: boolean;
  onUpgradeClick?: () => void;
};
            ...
            <div className="p-5 sm:p-6 border-b border-slate-200 dark:border-slate-800">
              ...
              <div className="grid sm:grid-cols-3 gap-3">
                <button
                  type="button"
                  onClick={() => {
                    if (hasAi) setAiOpen(true);
                    else onUpgradeClick?.();
                  }}
                  title={hasAi ? undefined : "Nâng cấp gói Pro để sử dụng AI"}
                  className={`flex items-center justify-center gap-2 rounded-xl border px-4 py-3 text-sm font-semibold transition ${
                    hasAi
                      ? "border-slate-200 dark:border-slate-800 bg-white dark:bg-slate-950 text-slate-800 dark:text-slate-100 hover:bg-slate-50 dark:hover:bg-slate-900"
                      : "border-amber-200 dark:border-amber-800 bg-amber-50/50 dark:bg-amber-500/5 text-amber-700 dark:text-amber-400 hover:bg-amber-100/50 dark:hover:bg-amber-500/10 cursor-pointer"
                  }`}
                >
                  {hasAi ? <Sparkles className="w-4 h-4 text-[#7C3AED]" /> : <Lock className="w-4 h-4" />}
                  Tạo trang với AI
                </button>
