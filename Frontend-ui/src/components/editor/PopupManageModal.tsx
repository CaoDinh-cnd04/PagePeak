...
export function PopupManageModal({ open, onClose, onPickBlank, onPickEntry }: PopupManageModalProps) {
  ...
  const templates = useMemo(() => filterPopupTemplates(categoryId), [categoryId]);
                  ...
                  myPopups.map((p) => {
                    ...
                    return (
                      <TemplateCard
                        ...
                        preview={<PopupMockPreview title={prev.title ?? "Popup"} body={prev.body ?? ""} styles={p.styles} />}
                        onPick={() => {
                          onPickEntry(p);
                          onClose();
                        }}
                ...
                {templates.map((t) => {
                  ...
                  return (
                    <TemplateCard
                      key={t.id}
                      title={t.name}
                      subtitle={POPUP_CATEGORIES.find((c) => c.id === t.category)?.label ?? t.category}
                      preview={
                        t.thumbnailUrl ? (
                          <img src={t.thumbnailUrl} alt="" className="w-full h-full object-cover" />
                        ) : (
                          <PopupMockPreview title={pop.title ?? "Popup"} body={pop.body ?? ""} styles={t.styles} />
                        )
                      }
                      onPick={() => {
                        onPickEntry(t);
...
function PopupMockPreview({ title, body, styles }: { title: string; body: string; styles: Record<string, string | number> }) {
  const bg = String(styles.backgroundColor ?? "#fff");
  const color = String(styles.color ?? "#334155");
  return (
    <div className="w-full h-full flex flex-col items-stretch justify-center p-3 text-left overflow-hidden" style={{ background: bg, color }}>
      <p className="text-[11px] font-bold truncate">{title}</p>
      <p className="text-[10px] opacity-90 line-clamp-3 mt-1 leading-snug">{body}</p>
    </div>
  );
}
