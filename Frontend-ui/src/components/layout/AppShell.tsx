...
type MenuDef = { key: MainMenuKey; i18nKey: string; icon: React.ReactNode; href: string; requiresEcommerce?: boolean };
...
export function AppShell({ children }: { children: React.ReactNode }) {
  ...
  const { hasEcommerce, hydrate } = usePlanStore();
  const t = useT();

  const MAIN_MENU = useMemo(
    () => ALL_MAIN_MENU.filter((m) => !m.requiresEcommerce || hasEcommerce),
    [hasEcommerce]
  );
  ...
  const [emailConfirmed] = useState<boolean | null>(null);
          ...
          <div className="mt-1 bg-white dark:bg-slate-900 border border-slate-200 dark:border-slate-700 rounded-lg shadow-lg p-1 z-50">
            ...
            <div className="border-t border-slate-200 dark:border-slate-700 mt-1 pt-1">
              <button
                ...
                onClick={() => { setWsDropdownOpen(false); navigate("/dashboard/pages"); }}
              >
        ...
        {!collapsed && (
          <div className="mt-4">
            <div className="flex items-center justify-between px-3 mb-1">
              ...
              <button type="button" className="w-5 h-5 rounded-full hover:bg-slate-200 dark:hover:bg-slate-700 flex items-center justify-center transition" onClick={() => navigate("/dashboard/integrations")}>
      ...
      >
        ...
        >
          ...
          <div className="h-14 px-3 flex items-center border-b border-slate-200 dark:border-slate-800 gap-2">
            <Link to="/dashboard/pages" className="flex items-center gap-2 hover:opacity-90 transition">
              <img
                src="/logo.jpg"
                alt="PagePeak"
      ...
      <div className="flex-1 min-w-0 flex flex-col">
        ...
        <header className="h-14 bg-white dark:bg-slate-950 border-b border-slate-200 dark:border-slate-800 flex items-center justify-between px-4 lg:px-6 relative z-30">
          ...
          <div className="flex items-center gap-2">
            ...
            <div ref={profileRef} className="relative">
              ...
              >
                {user?.avatarUrl ? (
                  <img src={user.avatarUrl} alt="" className="w-[34px] h-[34px] rounded-full object-cover" />
                ) : (
              ...
              {profileDropdownOpen && (
                <div className="absolute right-0 top-12 w-72 bg-white dark:bg-slate-900 border border-slate-200 dark:border-slate-700 rounded-xl shadow-xl z-[60] overflow-hidden">
                  ...
                  <div className="px-4 pt-4 pb-3 text-center border-b border-slate-100 dark:border-slate-800">
                    {user?.avatarUrl ? (
                      <img src={user.avatarUrl} alt="" className="w-14 h-14 rounded-full object-cover mx-auto mb-2 ring-2 ring-indigo-100 dark:ring-indigo-900" />
                    ) : (
                    ...
                    <button
                      ...
                      onClick={() => { setProfileDropdownOpen(false); navigate("/dashboard/settings?tab=verify"); }}
                    >
        ...
        <main className="flex-1 px-4 lg:px-6 py-6 overflow-auto">
          {emailConfirmed === false && (
            <div className="mb-4 flex items-center justify-between gap-4 rounded-xl border border-amber-200 dark:border-amber-500/30 bg-amber-50 dark:bg-amber-500/10 px-4 py-3">
              <div className="flex items-center gap-3">
                <Shield className="w-5 h-5 text-amber-600 dark:text-amber-400 shrink-0" />
                <div>
                  <p className="text-sm font-semibold text-amber-800 dark:text-amber-200">Chưa xác thực email</p>
                  <p className="text-xs text-amber-700 dark:text-amber-300/80">Vui lòng xác thực email để bảo mật tài khoản và nhận thông báo quan trọng.</p>
                </div>
              </div>
              <Link
                to="/dashboard/settings"
                className="shrink-0 px-4 py-2 text-sm font-medium rounded-lg bg-amber-600 hover:bg-amber-700 text-white transition"
              >
                Xác thực ngay
              </Link>
            </div>
