...
import { saveTokens, clearTokens, getAccessToken } from "@/lib/shared/auth";
...
export const useAuthStore = create<AuthState>((set, get) => ({
  ...
  fetchMe: async () => {
    ...
    try {
      ...
      // fetchWithAuth đã xóa token khi refresh thất bại; không gọi clearTokens thêm để tránh logout oan (lỗi mạng / thông điệp lỗi gây hiểu nhầm).
      if (!getAccessToken()) {
        set({ user: null });
      }
    }
