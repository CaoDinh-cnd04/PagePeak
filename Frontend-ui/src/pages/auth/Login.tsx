  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      if (msg === "EMAIL_VERIFICATION_REQUIRED" || msg === "EMAIL_NOT_VERIFIED") {
        navigate(`/verify-email?email=${encodeURIComponent(email)}&pending=1`);
        return;
      }
    ...
    <div className="min-h-screen flex flex-col bg-slate-50 dark:bg-slate-950">
        ...
        <div className="flex-1 flex items-center justify-center px-4 py-6 lg:py-8">
          <div className="w-full max-w-[420px] bg-white rounded-xl shadow-lg border border-slate-200 p-8 dark:bg-slate-900 dark:border-slate-800">
            ...
            <Link to="/" className="flex items-center justify-center gap-2 mb-8">
              <img src="/logo.jpg" alt="PagePeak" className="h-9 w-auto object-contain" style={{ width: "auto", height: "2.25rem" }} />
            ...
            <form onSubmit={handleSubmit} className="space-y-4">
              ...
              {!isLogin && (
                <label className="flex items-start gap-2 cursor-pointer">
                  ...
                  <span className="text-sm text-slate-600">
                    ...
                    <Link to="/#dieu-khoan" className="text-primary-600 hover:underline">Điều khoản sử dụng</Link>
                    {" & "}
                    <Link to="/#bao-mat" className="text-primary-600 hover:underline">Chính sách bảo mật</Link>
                    {" "}của PagePeak.
                  </span>
                </label>
              )}
              {isLogin && (
                <div className="text-right">
                  <Link to="/#lien-he" className="text-sm text-primary-600 hover:underline">
                    Quên mật khẩu?
                  </Link>
                </div>
              )}
            ...
            <p className="mt-6 text-center text-sm text-slate-600">
              {isLogin ? (
                <>
                  ...
                  <Link to="/login?mode=register" className="text-primary-600 font-medium hover:underline">
                    Đăng ký ngay
                  </Link>
                </>
              ) : (
      ...
      <footer className="py-4 text-center text-sm text-slate-500 dark:text-slate-400 border-t border-slate-200 dark:border-slate-800 bg-white dark:bg-slate-950">
        <div className="flex flex-wrap justify-center gap-4">
          <Link to="/#dieu-khoan" className="hover:text-slate-700 dark:hover:text-slate-200">Điều khoản sử dụng</Link>
          <Link to="/#bao-mat" className="hover:text-slate-700 dark:hover:text-slate-200">Chính sách bảo mật</Link>
          <Link to="/#lien-he" className="hover:text-slate-700 dark:hover:text-slate-200">Trợ giúp</Link>
        </div>
      </footer>
    </div>
  );
