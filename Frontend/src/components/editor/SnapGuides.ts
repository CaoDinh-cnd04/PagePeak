...
export function setupSnapGuides(
  ...
  canvas.on("object:moving", (e) => {
    ...
    if (Math.abs(tCenterX - centerX) < SNAP_THRESHOLD) {
      ...
      didSnapX = true;
      const line = createGuideLine(centerX, 0, centerX, canvasHeight);
    ...
    if (Math.abs(tCenterY - centerY) < SNAP_THRESHOLD) {
      ...
      didSnapY = true;
      const line = createGuideLine(0, centerY, canvasWidth, centerY);
      guideLines.push(line);
      canvas.add(line);
    }
    ...
    for (const obj of objects) {
      ...
      if (!didSnapX) {
        if (Math.abs(tLeft - oLeft) < SNAP_THRESHOLD) {
          ...
          const line = createGuideLine(oLeft, y1, oLeft, y2);
          guideLines.push(line);
          canvas.add(line);
          ...
          const y1 = Math.min(tTop, oTop);
          const y2 = Math.max(tBottom, oBottom);
          const line = createGuideLine(oRight, y1, oRight, y2);
          guideLines.push(line);
          canvas.add(line);
          ...
          const y1 = Math.min(tTop, oTop);
          const y2 = Math.max(tBottom, oBottom);
          const line = createGuideLine(oCenterX, y1, oCenterX, y2);
          guideLines.push(line);
          canvas.add(line);
      ...
      if (!didSnapY) {
        if (Math.abs(tTop - oTop) < SNAP_THRESHOLD) {
          ...
          const line = createGuideLine(x1, oTop, x2, oTop);
          guideLines.push(line);
          canvas.add(line);
          ...
          const x1 = Math.min(tLeft, oLeft);
          const x2 = Math.max(tRight, oRight);
          const line = createGuideLine(x1, oBottom, x2, oBottom);
          guideLines.push(line);
          canvas.add(line);
          ...
          const x1 = Math.min(tLeft, oLeft);
          const x2 = Math.max(tRight, oRight);
          const line = createGuideLine(x1, oCenterY, x2, oCenterY);
          guideLines.push(line);
          canvas.add(line);
